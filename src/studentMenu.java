...
public class studentMenu {
	...
	public static void viewTimeTable(int stuID) throws ClassNotFoundException, SQLException{
		...
		if(flag<i){ 			// 주어진 보기 번호가 아닌 다른 번호를 고를 경우 처리, 원래라면 다양하게 해줘야 하는데 그냥 더 큰 번호를 입력했을 때만 처리
			...
			ResultSet rs2=stmt2.executeQuery("select course_id, time_slot_id "
			...
			while(rs2.next()){
				...
				rs3=stmt3.executeQuery("select title from course where course_id = "+"'"+course+"'");
				/*time slot id를 이용해 (day, start_time, end_time) 추출*/
				rs4=stmt4.executeQuery("select day, start_hr,start_min,end_hr, end_min from time_slot where time_slot_id = "+"'"+tsid+"'");
				rs3.next();
